...
namespace FolderSync.SmokeTest
{
    ...
    internal static class TwoWayTests
    {
        ...
        public static async Task<int> RunAll()
        {
            ...
            await TestDirDeleteVsCreateInside();
        ...
        /// <summary>9) S-1 绞杀防线：右侧删除目录 + 左侧在目录内新建 → 目录行降级 Conflict 绝不自动传播。
        /// 修复前：目录删除无条件传播，build 趟先复制新建、postDelete 趟再连左侧原件整树删掉，
        /// 下一轮新建文件被判「对侧删除过」再传播 → 两侧全灭静默丢数据。</summary>
        private static async Task TestDirDeleteVsCreateInside()
        {
            var (root, left, right) = MakeDirs();
            try
            {
                Directory.CreateDirectory(Path.Combine(left, "D"));
                File.WriteAllText(Path.Combine(left, "D", "old.txt"), "old");
                using var db = new Db(Path.Combine(root, "test.db"));
                var job = NewJob(left, right, ConflictPolicy.NewestMtime, mirrorDelete: true);
                job.Id = db.InsertJob(job);
                var engine = new SyncEngine(job, db);
                await engine.RunAsync("manual", execute: true);   // 基线：两侧都有 D/old.txt
                Check(File.Exists(Path.Combine(right, "D", "old.txt")), "绞杀: 基线建立");

                // 右侧删除整个 D；左侧在 D 内新建 new.txt（快照无此文件）
                Directory.Delete(Path.Combine(right, "D"), true);
                File.WriteAllText(Path.Combine(left, "D", "new.txt"), "fresh content");

                var (_, plan) = await engine.RunAsync("manual", execute: true);
                Console.WriteLine($"绞杀防线计划: {string.Join("; ", plan.Select(p => $"{p.Action} {p.RelativePath} [{p.Note}]"))}");
                Check(plan.Any(p => p.Action == SyncAction.Conflict && p.RelativePath == "D"),
                    "绞杀: 目录行降级为 Conflict（人工裁决）");
                Check(!plan.Any(p => p.Action == SyncAction.DeleteLeft && p.RelativePath == "D"),
                    "绞杀: 不再产生目录整删传播动作");
                Check(plan.Any(p => p.Action == SyncAction.CreateRight && p.RelativePath == "D/new.txt"),
                    "绞杀: 目录内新建正常复制到右侧");
                // 执行后：新建文件两侧都在（修复前第二轮会把两侧的 new.txt 全部删光）
                Check(File.ReadAllText(Path.Combine(left, "D", "new.txt")) == "fresh content",
                    "绞杀: 左侧新建原件保留");
                Check(File.ReadAllText(Path.Combine(right, "D", "new.txt")) == "fresh content",
                    "绞杀: 右侧拿到新建副本");
                // 第三轮必须收敛：不再产生任何删除/复制（修复前 next 轮 DeleteRight(D/new.txt) 两侧全灭）
                var (_, plan3) = await engine.RunAsync("manual", execute: false);
                Check(plan3.Count == 0, "绞杀: 第三轮收敛无差异（无连锁删除）",

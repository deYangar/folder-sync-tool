    ...
    internal static class VerifyTests
    {
        ...
        private static async Task TestBitRotOneWayAutoFix()
        {
            ...
            try
            {
                ...
                var (rec, plan) = await engine.RunAsync("manual");   // 执行轮：位腐行+按源覆盖修复
                Check(rec.Status == "ok", "fix: 轮次 ok", rec.Status);
                Check(File.ReadAllText(Path.Combine(right, "f.txt")) == "权威内容",
                    "fix: 单向位腐按源覆盖自动修复");
                var (_, plan2) = await engine.RunAsync("manual", execute: false);
                Check(plan2.Count == 0, "fix: 修复后收敛", plan2.Count.ToString());
            }
            finally { Cleanup(root, db); }
        }

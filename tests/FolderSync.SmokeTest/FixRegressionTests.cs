...
namespace FolderSync.SmokeTest
{
    ...
    internal static class FixRegressionTests
    {
        ...
        /// <summary>2026-09-21 review C-3/C-4：errno→Win32 单表关键值。
        /// EDQUOT(122)→395 保证 Linux 配额满走磁盘满熔断；ENOTEMPTY/ELOOP 用 winerror.h 正确值；
        /// BclFileOps 与平台实现共用 Errno.ToWin32 单表（宿主装不装 Platforms 判定一致）。</summary>
        private static void TestErrnoMapping()
        {
            static int Map(int errno) =>
                Core.Platform.Errno.ToWin32(new IOException("x", unchecked((int)0x80070000) | errno));
            Check(Map(28) == 112, "errno: ENOSPC→112 磁盘满熔断");
            Check(Map(122) == 395, "errno: EDQUOT→395 配额满同走熔断（原 122 映不到）");
            Check(Map(39) == 145, "errno: ENOTEMPTY→145（原 116 是 ERROR_NO_SIGNAL_SENT 错表）");
            Check(Map(40) == 1142, "errno: ELOOP→1142 TOO_MANY_LINKS（原 232 是 ERROR_NO_DATA 错表）");
            Check(Map(18) == 17, "errno: EXDEV→17 NOT_SAME_DEVICE");
            Check(Map(20) == 2, "errno: ENOTDIR→2 按不存在处理");
            Check(Map(11) == 32, "errno: EAGAIN→32 共享冲突（瞬时重试类）");
        }
        ...
        private static async Task TestTmpSweepAndNoBackflow()
        {
            ...
            try
            {
                ...
                // C-6 起 60s 内的 tmp 属并发豁免窗（另一任务可能正在写）——崩溃残留形态的
                // mtime 必然久远（上轮及更早），测试模拟 61 秒前
                ...
                foreach (var t in new[]
                         {
                             Path.Combine(right, "sub", "junk.txt" + Scanner.TmpSuffix),
                             Path.Combine(right, "root.foldersync-tmp")
                         })
                    File.SetLastWriteTimeUtc(t, DateTime.UtcNow.AddSeconds(-61));
                ...
                // 过去 2 小时：越过 C-6 的 60s 并发豁免窗（半截残留的 mtime 必然久远）
                File.SetLastWriteTimeUtc(Path.Combine(right, "big.bin.foldersync-tmp"), DateTime.UtcNow.AddHours(-2));

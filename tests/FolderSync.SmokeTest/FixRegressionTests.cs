    ...
    internal static class FixRegressionTests
    {
        ...
        private static async Task TestHotplugMedia()
        {
            ...
            try
            {
                ...
                Check(File.Exists(Path.Combine(right, "during-gone.txt")),
                    "热插拔: 补跑把消失期间的写入同步到位");
            }
            finally
            {
        ...
        private static async Task TestFailedItemsAndRetry()
        {
            ...
            Directory.CreateDirectory(left);
            Directory.CreateDirectory(right);
            var logDir = Path.Combine(AppContext.BaseDirectory, "logs", "failed", "job0");
            try
            {
                ...
                // 独占句柄锁住目标：rename 替换被拒 → 该条失败，其余正常
                using (var lockFs = new FileStream(Path.Combine(right, "locked.txt"),
        ...
        private static void TestRootOfPathBoundary()
        {
            ...
            // 前缀兄弟目录：短根是长根的字符串前缀
            var e1 = new Executor(new SyncJob { LeftPath = @"D:\Doc", RightPath = @"D:\Documents" });
            Check(RootOf(e1, @"D:\Documents\a.txt") == @"D:\Documents", "RootOf: 前缀兄弟目录右根文件判右（修复前误判左）");
            Check(RootOf(e1, @"D:\Doc\a.txt") == @"D:\Doc", "RootOf: 左根文件判左");
            Check(RootOf(e1, @"D:\Doc") == @"D:\Doc", "RootOf: 根路径自身判该侧");

            // 左右互换（长根在左）
            var e2 = new Executor(new SyncJob { LeftPath = @"D:\Documents", RightPath = @"D:\Doc" });
            Check(RootOf(e2, @"D:\Documents\a.txt") == @"D:\Documents", "RootOf: 长左根文件判左");
            Check(RootOf(e2, @"D:\Doc\a.txt") == @"D:\Doc", "RootOf: 短右根文件判右（修复前误判左）");

            // 尾分隔符形态（用户配置路径带尾杠）
            var e3 = new Executor(new SyncJob { LeftPath = @"D:\Doc\", RightPath = @"D:\Documents\" });
            Check(RootOf(e3, @"D:\Documents\a.txt") == @"D:\Documents\", "RootOf: 尾杠形态判右");
            Check(RootOf(e3, @"D:\Doc\a.txt") == @"D:\Doc\", "RootOf: 尾杠形态判左");

            // 常规不相关兄弟目录（回归确认不受影响）
            var e4 = new Executor(new SyncJob { LeftPath = @"E:\L", RightPath = @"E:\R" });
            Check(RootOf(e4, @"E:\R\x") == @"E:\R" && RootOf(e4, @"E:\L\x") == @"E:\L",
                "RootOf: 常规兄弟目录不受影响");
        }
    }
}

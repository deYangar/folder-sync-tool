...
namespace FolderSync.SmokeTest
{
    ...
    internal static class RetryTests
    {
        ...
        /// <summary>1：源文件被独占锁（FileShare.None）→ CopyFileEx win32 32 进队列；
        /// 主线程 250ms 后释放 → 第 2/3 轮重试成功。retried_ok=1、failed=0、runs ok。</summary>
        private static async Task TestLockedThenRelease()
        {
            var (root, left, right, job, db, engine) = Setup("release");
            try
            {
                var srcLock = new FileStream(Path.Combine(left, "locked.txt"), FileMode.Open, FileAccess.Read, FileShare.None);
                var round = Task.Run(() => engine.RunAsync("manual"));
                Thread.Sleep(250);   // 主趟失败进队列（100ms 第一轮大概率也撞锁），此后释放
                srcLock.Dispose();
        ...
        private static async Task TestLockedForever()
        {
            ...
            try
            {
                using var srcLock = new FileStream(Path.Combine(left, "locked.txt"), FileMode.Open, FileAccess.Read, FileShare.None);

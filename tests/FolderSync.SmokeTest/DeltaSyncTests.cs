...
WHERE rel_path=@r AND seq IN (1,2);", new { r = rel });
        ...
        private static async Task TestE1ArchiveFailNoBaseline()
        {
            ...
            try
            {
                ...
                // 版本库中心根指向必败路径 → ArchiveFile 必败 → E1 不建基线（keep=3 走真归档失败路径）。
                // 用「已存在文件占位」制造跨平台一致的失败（曾用 bad<|>root：Windows 非法字符 Unix 全合法，
                // Unix 上目录真建出来、归档成功，⑮-3 误红）
                var blocker = Path.Combine(root, "bad-root-blocker");
                File.WriteAllText(blocker, "x");
                VersionStore.CentralRootOverride = blocker;
                var job = new SyncJob
                {

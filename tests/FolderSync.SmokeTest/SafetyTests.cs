...
namespace FolderSync.SmokeTest
{
    ...
    internal static class SafetyTests
    {
        ...
        private static async Task TestCountConservationWithRecycleFailure()
        {
            ...
            // 制造删除计划：目标新增 3 个孤儿（低于豁免线）+ 其中 1 个被独占打开（回收站删除必失败）
            for (int i = 0; i < 3; i++) Touch(Path.Combine(right, $"locked{i}.txt"), "l" + i);
            var handle = new FileStream(Path.Combine(right, "locked1.txt"), FileMode.Open,
                FileAccess.Read, FileShare.Read);   // 无 Delete 共享 → SHFileOperation 删不掉
        ...
        private static void TestPathGuards()
        {
            ...
            Check(msg2.Contains("程序目录"), "任务侧覆盖程序目录被运行时守卫拒绝（ExecutePlanAsync 入口，防数据自反馈）", msg2);

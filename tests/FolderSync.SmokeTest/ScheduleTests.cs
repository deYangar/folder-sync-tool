...
namespace FolderSync.SmokeTest
{
    ...
    internal static class ScheduleTests
    {
        ...
        {
            TestNextDueMatrix();
            TestTickFiresAtDuePoint();
            await TestBusySkip();
            TestInvalidSpecIgnored();
            TestDapperRoundTrip();
        ...
        private static void TestNextDueMatrix()
        {
            ...
            Check(ScheduleSpec.NextDue("daily 03:00", wed10) == new DateTime(2026, 9, 17, 3, 0, 0),
                "daily 已过 → 明天同一时刻", ScheduleSpec.NextDue("daily 03:00", wed10)?.ToString("yyyy-MM-dd HH:mm") ?? "(null)");
            ...
            Check(ScheduleSpec.NextDue("daily 10:00", wed10) == new DateTime(2026, 9, 17, 10, 0, 0),
                ...
                ScheduleSpec.NextDue("daily 10:00", wed10)?.ToString("MM-dd HH:mm") ?? "(null)");
            ...
            Check(ScheduleSpec.NextDue("weekly 1,3,5 15:00", wed10.AddHours(6)) == new DateTime(2026, 9, 18, 15, 0, 0),
                ...
                ScheduleSpec.NextDue("weekly 1,3,5 15:00", wed10.AddHours(6))?.ToString("MM-dd HH:mm") ?? "(null)");
            ...
            Check(ScheduleSpec.NextDue("weekly 7 08:00", new DateTime(2026, 9, 18, 16, 0, 0)) == new DateTime(2026, 9, 20, 8, 0, 0),
                "weekly 7=周日边界", ScheduleSpec.NextDue("weekly 7 08:00", new DateTime(2026, 9, 18, 16, 0, 0))?.ToString("MM-dd HH:mm") ?? "(null)");
        ...
        private static void TestTickFiresAtDuePoint()
        {
        ...
        private static async Task TestBusySkip()
        {
            ...
            try
            {
                ...
                // 拦住 a.txt 的复制让 manual 轮持续忙（FileShare.None 在 Unix 无强制力，统一 FileOps 注入）
                using (var hold = TestFileOps.BlockCopy(Path.Combine(left, "a.txt")))
                {
                ...
                }   // 释放 → manual 轮复制完成收尾（Block 语义下无失败重试）
        ...
        private static void TestInvalidSpecIgnored()
        {
        ...
        private static void TestDapperRoundTrip()
        {
            ...
            try
            {
                ...
                using (var db2 = new Db(dbPath))
                {
                    ...
                    Check(j?.Trigger == TriggerType.Schedule, "db: trigger=3 往返", j?.Trigger.ToString() ?? "(null)");

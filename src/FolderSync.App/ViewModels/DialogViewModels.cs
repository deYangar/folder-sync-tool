...
public class JobEditViewModel : ViewModelBase
{
    ...
    public int VersionKeepCount => int.TryParse(VersionKeepText.Trim(), System.Globalization.CultureInfo.InvariantCulture, out var n)
...
public class HistoryViewModel : ViewModelBase
{
    ...
    public string ExportCsv(string file)
    {
        var lines = new[] { "开始,触发,状态,复制,删除,跳过,失败,字节,增量省,耗时s,均速,错误" }
            .Concat(Rows.Select(r => $"{r.StartedText},{r.TriggerText},{r.StatusText},{r.Record.CopiedFiles},{r.Record.DeletedFiles},{r.Record.SkippedFiles},{r.Record.FailedFiles},{r.Record.BytesCopied},{r.Record.DeltaSavedBytes},{r.DurationText},{r.SpeedText},\"{r.ErrorText.Replace("\"", "\"\"")}\""));

...
internal static class Cli
{
    ...
    private static TextWriter? AttachWriters()
    {
        try
        {
            // 输出已重定向（管道/文件，测试与 CI 场景）：直接写 UTF-8 管道——绝不能先碰
            // Console.OutputEncoding（WinExe 子进程无控制台句柄时设它抛 IOException，被
            // 吞掉后整个 CLI 输出静默丢失，2026-09-16 cli 门禁实测坑）
...
#endif
            return new StreamWriter(Console.OpenStandardOutput(), Encoding.UTF8) { AutoFlush = true };
        }
        catch { return null; }
    }

...
public sealed class TrayService : IDisposable
{
    ...
    // ---------- G-6：托盘可用性探测 ----------
    // GNOME 默认（无 StatusNotifier 扩展）无系统托盘，Avalonia TrayIcon 静默不显示——
    // 此时「关窗藏托盘」「--minimized 最小化启动」都会让主窗口永久失联（只能杀进程）。
    // 探测 = 会话 DBus 上 org.kde.StatusNotifierWatcher 名字是否有人应答（KDE/XFCE/装了扩展的
    // GNOME 都会注册）；gdbus 是 glib 自带工具， GNOME 系必有。探测失败按「不可用」保守处理。
    private static bool? _trayLikelyAvailable;

    public static bool TrayLikelyAvailable
    {
        get
        {
            if (!OperatingSystem.IsLinux()) return true;   // Win/macOS 托盘系统级必有
            if (_trayLikelyAvailable.HasValue) return _trayLikelyAvailable.Value;
            _trayLikelyAvailable = ProbeStatusNotifier();
            return _trayLikelyAvailable.Value;
        }
    }

    private static bool ProbeStatusNotifier()
    {
        try
        {
            ...
            psi.ArgumentList.Add("org.kde.StatusNotifierWatcher");
            using var p = System.Diagnostics.Process.Start(psi)!;
            var output = p.StandardOutput.ReadToEnd();
            if (!p.WaitForExit(3_000)) try { p.Kill(); } catch { }
            return p.ExitCode == 0 && output.Contains("true", StringComparison.OrdinalIgnoreCase);
        }
        catch { return false; }   // gdbus 缺席等：按无托盘保守处理（宁可真退出，不永久失联）
    }

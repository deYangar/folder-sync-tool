    ...
    public class SyncEngine : IDisposable
    {
        ...
        public void ApplyTriggers()
        {
            ...
            if (Job.Trigger == TriggerType.Realtime)
            {
                ...
                foreach (var watchRoot in roots)
                {
                    try
                    {
                        fresh.Add(new UsnWatcher(watchRoot, Job.ExcludePatterns,
                            onChange: () => { Interlocked.Increment(ref _watcherFireCount); OnWatchedChange(); },
                            // USN 日志回绕/禁用/重置会跳过中间的变更记录：
                        ...
                        StatusText = "实时监听中（USN）";
                    }
                    catch
                    {
                        try
                        {
                            fresh.Add(new DirWatcher(watchRoot, Job.ExcludePatterns,
                                onChange: () => OnWatchedChange(),
                                // FSW 缓冲区溢出 = 丢过事件，同样要全量校准
                                onOverflow: () => OnWatchedChange()));
                            StatusText = "实时监听中（FSW 兜底）";
                        }
                        catch (Exception ex)
                        {
            ...
            Func<RunRecord, CancellationToken, IProgress<ProgressInfo>, Task<(List<PlanEntry> plan, string finalStatusText)>> body)
        {
            ...
            try
            {
                ...
                _jobRunMutex = TryAcquireJobMutex();
                if (_jobRunMutex == null)
            ...
            {
                _jobRunMutex?.Dispose();   // 关句柄即释放跨进程互斥（线程亲和约束下不走 ReleaseMutex）
                _jobRunMutex = null;
        ...
        private Mutex? _jobRunMutex;

        /// <summary>抢同任务的全局执行互斥（null=另一进程/实例正在跑该任务）。
        /// AbandonedMutexException（上个持有进程被强杀）时本调用已获得所有权，照常持有。</summary>
        private Mutex? TryAcquireJobMutex()
        {
        ...
        // 对比阶段：大任务树差异计算可能耗时数秒，明确报阶段避免 UI 假死
                ...
                if (Job.DeepVerify > 0 && execute)
                {
                    ...
                    if (rot.Count > 0)
                    {
                        ...
                        LastWarning = $"深度校验发现 {rot.Count} 处位腐差异（大小/时间一致但内容不同），已标记待处理";
                    }
                }
        ...
        /// 分析/预览轮（logRun=false）不该让托盘进入"同步中"状态。每轮 RunAsync 开头写入，
        /// 只在 IsRunning=true 时读取有意义，无需收尾复位。</summary>
        ...
        /// <summary>三个执行入口共用的路径守卫：空路径 / 路径无效 / 两侧互嵌套 / 侧与程序目录重叠。
        /// 互嵌套与程序目录重叠在 UI 编辑时已挡（JobEditWindow），这里做运行时二次校验——
        /// 防直接改库绕过 UI（B18）与 junction/目录移动造成的关系漂移。</summary>
        private void ValidatePathsForRun()
        {
            ...
            try
            {
                l = Path.GetFullPath(Job.LeftPath).TrimEnd('\\') + "\\";
                r = Path.GetFullPath(Job.RightPath).TrimEnd('\\') + "\\";
                appRoot = Path.GetFullPath(AppContext.BaseDirectory).TrimEnd('\\') + "\\";
            }
            catch (Exception ex)
            {
            ...
            if (l.StartsWith(appRoot, StringComparison.OrdinalIgnoreCase) || r.StartsWith(appRoot, StringComparison.OrdinalIgnoreCase)
                ...
                throw new InvalidOperationException("任务侧与程序目录重叠（同步程序数据会自反馈：数据库/版本库边写边产生新差异），请调整路径");
        }
        ...
        private void CaptureFailures(RunRecord record, Executor executor)
        {
            ...
            try
            {
                var dir = Path.Combine(AppContext.BaseDirectory, "logs", "failed", $"job{Job.Id}");
        ...
        private void WriteStateFile(ProgressInfo p)
        {
            ...
            var file = Path.Combine(AppContext.BaseDirectory, "logs", $"job{Job.Id}.state.txt");
            Task.Run(() =>
            {
                try
                {
        ...
        /// 不执行任何写动作；结果=位腐冲突行（树中展示，双向人工裁决/单向下轮同步按源覆盖修复）。</summary>

...
namespace FolderSync.Core.Platform.Bcl
{
    ...
    internal sealed class BclFileOps : IFileOps
    {
        public bool CopyFile(string src, string dst, Func<long, long, bool>? onProgress, out int win32Error)
        {
            ...
            catch (IOException io) { win32Error = Win32ErrorOf(io); return false; }
            catch (Exception) { win32Error = 1; return false; }
        }

        /// <summary>IOException → 近似 Win32 码：ENOSPC(28)→112 磁盘满是关键映射，
        /// 保证 Executor 的 IsDiskFullEx 熔断语义在无平台实现时依然成立。</summary>
        internal static int Win32ErrorOf(IOException io) => (io.HResult & 0xFFFF) switch
        {
            28 => 112,    // ENOSPC → ERROR_DISK_FULL
            13 => 5,      // EACCES → ERROR_ACCESS_DENIED
            2 => 2, 17 => 183, 20 => 32, 21 => 32, 22 => 123,
            30 => 26, 36 => 206, 39 => 116, 40 => 232, _ => 1
        };
    ...
    /// Linux=inotify、macOS=FSEvents，本来就是跨平台的）。</summary>

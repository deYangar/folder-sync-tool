            ...
            foreach (var root in roots)
            {
                ...
                try
                {
                    ...
                    foreach (var fi in new DirectoryInfo(root).EnumerateFiles("*" + Scanner.TmpSuffix, opt))
                    {
                        try
                        {
                            // C-6：另一并发任务正在写的 tmp 豁免——Unix 上 unlink 对打开中的文件直接
                            // 成功，任务 A 写完 rename 时 tmp 已被 B 清掉、白失败一轮。
                            // 崩溃残留的 tmp 的 mtime 必然久远（上轮及更早），60s 豁免窗不漏扫
                            if (DateTime.UtcNow - fi.LastWriteTimeUtc < TimeSpan.FromSeconds(60)) continue;
                            fi.Attributes = FileAttributes.Normal; fi.Delete();
                        }
                        catch { /* 正被占用等：留给下轮 */ }
                    }
                }
        ...
        private (bool ok, long bytes) ExecuteEntry(PlanEntry entry)
        {
            ...
            switch (entry.Action)
            {
                ...
                case SyncAction.Conflict:
                    ...
                    // 参考#1：位腐行（深度校验产出）在单向也不自动覆盖——源侧未必是好方，
                    // 按源覆盖等于抛硬币，源是坏方时毁掉唯一好副本。好坏未知交给人工：手动删除坏侧文件再同步即修复
                    if (entry.BitRot) return (false, 0);

...
{
    /// <summary>同步执行器：CopyFileEx 流式复制 + 进度/速率/ETA + 回收站删除。</summary>
    public class Executor
    {
        ...
        /// <summary>把源的创建/修改时间写到 tmp（尽力而为：失败不影响复制主体，mtime 另有 CopyFileEx 原生兜底）。</summary>
        private static void CopyTimestamps(string srcL, string tmpL)
        {
            try
            {
                if (!GetFileAttributesEx(srcL, 0, out var d)) return;
                using var h = CreateFileW(tmpL, FILE_WRITE_ATTRIBUTES, 7, IntPtr.Zero, 3, 0, IntPtr.Zero);
        ...
        /// <summary>path 是否在 root 之下（带路径边界：D:\Doc 不吞 D:\Documents 这类字符串前缀兄弟目录）。</summary>
        ...
        private void CopyOne(string src, string dst, Action? archiveOldTarget = null)
        {
            ...
            try
            {
                bool ok = CopyFileEx(srcL, tmpL, (total, transferred, _, _, _, _, _, _, _) =>
                {
                    ...
                    // 取消即时生效：token 触发（引擎 RequestCancel/UI _cts）立刻中止 CopyFileEx，不拷完整个文件
                    if (_cancel || _ct.IsCancellationRequested) { _cancel = true; return 1; }
                    return 0; // PROGRESS_CANCEL=1, PROGRESS_CONTINUE=0（WinBase.h 定义，别记反！）
                }, IntPtr.Zero, ref cancelFlag, 0);

                if (!ok)
                {
                    var err = Marshal.GetLastWin32Error();
                    if (_cancel) throw new OperationCanceledException("用户取消");
                    if (cancelFlag) throw new OperationCanceledException($"CopyFileEx 取消标志异常 (win32={err})");
                    throw new Win32Exception(err, $"复制失败 (Win32 {err})");
                }
                // 中转件完整：带上源 mtime/ctime（创建时间），再原子替换目标（旧版本任何时刻都在）。
                // mtime 另有 CopyFileEx 原生复制兜底；ctime 必须显式设（CopyFileEx 落地的 tmp 创建时间=复制时刻），
                // rename 不改 ctime，设完再挪即保留
                CopyTimestamps(srcL, tmpL);
        ...
        private bool DeleteOne(string path, bool isDirectory, SyncAction origAction)
        {
        ...
        /// <summary>刷盘：攒批的回收站文件删除拼一次 SHFileOperation（pFrom 多路径以 \0 分隔、整体双 \0 结尾），随后核对。</summary>
        ...
        private void FlushRecycleLocked()
        {
            ...
            try
            {
                ...
                {
                    wFunc = FO_DELETE,
                    pFrom = string.Join("", batch.Select(p => p.path + "\0")) + "\0",   // 多路径：每个 \0 结尾 + 整体再补 \0
                    fFlags = FOF_ALLOWUNDO | FOF_NOCONFIRMATION | FOF_SILENT | FOF_NOERRORUI
                };
                SHFileOperation(ref op);
            }
            catch { /* 批调用异常不致命，下面逐个核对兜底 */ }
            foreach (var (p, origAction) in batch)
            {
                if (!File.Exists(LongPath(p))) continue;   // 已删掉
                try { RecycleBinDelete(p, isDirectory: false); } catch { }
        ...
        /// <summary>单文件回收站删除。SHFileOperation 官方不保证线程安全——并行 worker 的批删/目录直删
        /// 统一经 _recycleGate 串行（FlushRecycleLocked 内已持锁调用，Monitor 可重入不嵌套阻塞）。</summary>
        private void RecycleBinDelete(string path, bool isDirectory)
        {
            lock (_recycleGate)
            {
                ...
                var stillThere = isDirectory ? Directory.Exists(LongPath(path)) : File.Exists(LongPath(path));
                if (stillThere && rc != 0 && !op.fAnyOperationsAborted)
                    throw new IOException($"删除失败 (SHFileOperation {rc}): {path}");
            }
        }

        /// <summary>网络路径判定：\\ 前缀直接判；映射盘符（Z:\）经 WNetGetConnection 解析真身——
        /// 映射盘没有回收站，FOF_ALLOWUNDO 会静默永久删除，必须按网络路径同等处理。</summary>
        ...
        internal static string LongPath(string path)
        {
            ...
            if (path.StartsWith(@"\\UNC\")) return @"\\?\UNC\" + path[6..];   // 历史错写防御

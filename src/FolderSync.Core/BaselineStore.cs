...
namespace FolderSync.Core
{
    ...
    /// <summary>
    /// 块级增量同步的基线缓存（v1.4）：<c>&lt;程序目录&gt;\sync-index\&lt;侧根哈希16位&gt;\baseline.db</c>，
    ...
    public static class BaselineStore
    {
        ...
        /// <summary>测试注入的根（null=程序目录下 sync-index）。必须在首次使用 BaselineStore 前设置。</summary>
        internal static string? RootOverride;

        public static string RootBase => RootOverride ?? Path.Combine(AppContext.BaseDirectory, "sync-index");

        /// <summary>侧根 → 基线库键：SHA-256(去尾反斜杠+大写不变文化)[..16]，与版本库同规则。</summary>

...
namespace FolderSync.Core
{
    ...
    public static class Scanner
    {
        ...
        /// <summary>程序数据根（exe 目录，含 foldersync.db/versions/sync-index/logs/crash.log）：
        /// 任务侧覆盖程序目录时整个子树排除——同步程序自身文件无意义，且同步中写入的 db/块文件
        /// 会立刻成为新差异，形成永不收敛的自反馈循环。</summary>
        private static readonly string AppDataRoot =
            Path.GetFullPath(AppContext.BaseDirectory).TrimEnd('\\') + "\\";

        /// <summary>fullPath 是否位于程序数据目录之下（含整体等于）。</summary>

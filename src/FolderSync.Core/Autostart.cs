...
{
    /// <summary>
    /// 开机自启：计划任务（schtasks ONLOGON /rl HIGHEST）。
    ...
    public static class Autostart
    {
        ...
        private static (int rc, string stderr) RunSchtasks(string arguments)
        {
            ...
            // schtasks 卡死防御（计划服务无响应等）：30s 放弃
            if (!p.WaitForExit(30_000)) try { p.Kill(); } catch { }
            p.WaitForExit(); // 异步输出收尾，否则 ExitCode 可能抛异常、stderr 可能截断
        ...
        private static string? QueryXml()
        {
            ...
            p.WaitForExit();   // 异步输出收尾，否则 ExitCode 可能抛异常
        ...
        public static void Enable(bool startMinimized = true)
        {
            ...
            if (rc != 0)
                throw new InvalidOperationException($"创建计划任务失败 (schtasks 退出码 {rc}): {err}");
        ...
        public static void Disable()
        {
            ...
            // 只有任务仍在才把失败抛出去
            if (rc != 0 && IsEnabled())
                throw new InvalidOperationException($"删除计划任务失败 (schtasks 退出码 {rc}): {err}");
        }

        public static bool IsEnabled() => RunSchtasks($"/query /tn {TaskName}").rc == 0;
        ...
        public static bool IsMinimized()
        {
            ...
            return xml.Contains("--minimized", StringComparison.OrdinalIgnoreCase);
        }
        ...
        public static void EnsureUpToDate()
        {
            ...
            var xml = QueryXml();
            var exe = Environment.ProcessPath;
            if (xml != null && exe != null && !xml.Contains(exe, StringComparison.OrdinalIgnoreCase))
                Enable(IsMinimized());
        }

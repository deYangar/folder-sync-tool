...
namespace FolderSync.Core
{
    ...
    /// <summary>
    /// 块级去重版本库（v1.3）：中心位置 <c>&lt;程序目录&gt;\versions\&lt;侧根哈希16位&gt;\repo\</c> 下
    ...
    public static class VersionStore
    {
        ...
        /// <summary>测试注入的中心根（null=程序目录下 versions）。必须在首次使用 VersionStore 前设置。</summary>
        ...
        public static string CentralRoot => CentralRootOverride ?? Path.Combine(AppContext.BaseDirectory, "versions");

        /// <summary>侧根 → 中心库键：SHA-256(去尾反斜杠+大写不变文化)[..16]，同侧大小写不敏感稳定。</summary>
        ...
        /// <summary>库根固定在程序目录中心位置（同步文件夹零数据夹）。
        ...
        private static string LongPath(string p) => p.StartsWith(@"\\?\") ? p : (p.StartsWith(@"\\") ? @"\\?\UNC\" + p[2..] : @"\\?\" + p);
                ...
                using (var sha = SHA256.Create())
                {
                        ...
                        _chunkBuf ??= new byte[CdcChunker.MaxChunkSize], _readBuf ??= new byte[80 * 1024]))
                    {
                        ...
                        if (!File.Exists(LongPath(objPath)))
                        {
                            ...
                            var tmp = objPath + Scanner.TmpSuffix;

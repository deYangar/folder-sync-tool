...
namespace FolderSync.Platforms.Unix
{
    ...
    public sealed class XdgAutostart : IAutostart
    {
        ...
        /// <summary>desktop 规范 Exec 的引号转义（U-4，按 Desktop Entry Spec 1.5）：
        /// 保留字符 $ ` \ " ' 反斜杠转义（解析器 shell-like 展开）、% 双写防 field code
        /// （路径含 % 时 GNOME 判条目非法静默忽略）、含空格整体引号（引号内空格不再分词）。
        /// 转义顺序：先反斜杠自身，再其余保留字符，最后 %（%% 不会被前面的转义波及）。</summary>
        private static string ExecValue(string exe, bool minimized)
        {
            var esc = exe.Replace("\\", "\\\\")
                         ...
                         .Replace("'", "\\'")
                         .Replace("%", "%%");
            var quoted = esc.Contains(' ') ? "\"" + esc + "\"" : esc;

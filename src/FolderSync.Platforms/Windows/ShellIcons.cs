...
namespace FolderSync.Platforms.Windows
{
    ...
    internal sealed class ShellIcons : IIconProvider
    {
        ...
        // W-5①：扩展名→「有缩略图能力」只记 true——负判定不固化（首个文件恰逢缩略图缓存冷/
        // 文件损坏会把整扩展名永久误判成无缩略图不可逆），失败下次重试，代价仅一次 COM 探测
        private readonly HashSet<string> _thumbExtOk = new(StringComparer.OrdinalIgnoreCase);
        private readonly Dictionary<string, byte[]> _thumbCache = new(StringComparer.OrdinalIgnoreCase); // 路径:像素→缩略图 PNG
        ...
        private byte[]? ForFile(string path, int pixelSize)
        {
            ...
            // 1) 内容缩略图：扩展名已知有缩略图能力的类型逐文件取（键含 pixelSize——W-5②：不同尺寸
            // 的缩略图不能混用同一键，否则 16px 视图拿到 256px 图、反之糊成一团）
            if (ext.Length > 0 && _thumbExtOk.Contains(ext))
            {
                var key = path + ":" + pixelSize;
                if (_thumbCache.TryGetValue(key, out var cached)) return cached;
                if (TryThumbnail(path, pixelSize, out var png)) { Trim(_thumbCache, key, png!); return png; }
            }
            else if (ext.Length > 0)
            {
                // 首次见该扩展名：探测一次，成功才固化「有缩略图」（失败不固化，见 _thumbExtOk 注释）
                if (TryThumbnail(path, pixelSize, out var probed))
                {
                    _thumbExtOk.Add(ext);
                    Trim(_thumbCache, path + ":" + pixelSize, probed!);
                    return probed;
                }
            }
        ...
        /// <summary>路径级缓存上限纪律（WPF 版 PathCacheLimit 同语义）：涨到上限整体清空重建。
        /// null 不缓存（W-5③ 与探测失败不固化的纪律配套）：文件内容/关联可能变化，下次重试。</summary>
